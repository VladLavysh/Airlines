<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold">Airlines</h1>
      <UButton color="primary" icon="i-lucide-plus" @click="openCreate">New Airline</UButton>
    </div>
...
const columns = [
  ...
  { accessorKey: 'iata_code', header: 'IATA' },
...
const form = reactive({ name: '', iata_code: '', country: '', price_multiplier: 1.0 });

async function fetchItems() {
  loading.value = true;
  try {
    items.value = await api<any[]>('/airline', { query: { limit: pagination.limit, offset: pagination.offset } });
  } catch { items.value = []; }
...
function openCreate() {
  editing.value = null;
  Object.assign(form, { name: '', iata_code: '', country: '', price_multiplier: 1.0 });
  formError.value = '';
  showForm.value = true;
}
...
function openEdit(item: any) {
  ...
  Object.assign(form, { name: item.name, iata_code: item.iata_code, country: item.country, price_multiplier: Number(item.price_multiplier) });
  formError.value = '';
  showForm.value = true;
}
...
async function save() {
  ...
  try {
    if (editing.value) {
      await api(`/airline/${editing.value}`, { method: 'PATCH', body: { ...form } });
      toast.add({ title: 'Airline updated', color: 'success' });
    } else {
      await api('/airline', { method: 'POST', body: { ...form } });
      toast.add({ title: 'Airline created', color: 'success' });
    }
...
async function confirmDelete(item: any) {
  if (!confirm(`Delete airline "${item.name}"?`)) return;
  try {
    await api(`/airline/${item.id}`, { method: 'DELETE' });
    toast.add({ title: 'Airline deleted', color: 'success' });
    fetchItems();
  } catch (e: any) {
